...
def fetch_posts(posts_list_path: Path = POSTS_LIST_PATH):
    posts_list_path.parent.mkdir(exist_ok=True)
...
def get_metadata_mapping() -> dict[int, str]:
...
def download_python_snippets(
    posts_dir: Path = POSTS_DIR, num_threads: Optional[int] = None
):
...
def parse_args():
    ...
    parser.add_argument(
        "--download",
        action="store_true",
        help="Download code snippets from fetched post IDs.",
    )
